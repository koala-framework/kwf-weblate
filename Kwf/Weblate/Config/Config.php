...
class Config implements ConfigInterface
{
    ...
    public function __construct()
    {
        if (isset($_ENV['KWF_LINGOHUB_APITOKEN'])) {
            ...
            $path = $this->_getHomeDir().'/koala-framework/kwf-weblate/config';
            if (!file_exists($path)) {
                throw new \Exception("No kwf-weblate config found! ($path)");
            }
    ...
    */
    private function _getHomeDir()
    {
        if (defined('PHP_WINDOWS_VERSION_MAJOR')) {
            if (!getenv('APPDATA')) {
                throw new \Exception("The APPDATA environment variable must be set for kwf-weblate to run correctly");
            }
            $home = strtr(getenv('APPDATA'), '\\', '/') . '/Config';
        } else {
            if (!getenv('HOME')) {
                throw new \Exception("The HOME environment variable must be set for kwf-weblate to run correctly");
            }
